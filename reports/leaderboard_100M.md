# 100M Leaderboard — ratio-10 (γ=10), matched 21B-byte budget

All models trained at **100M params, ratio-10** (γ=10 ≈ 21B bytes). Metric is the
**final-window training loss** `loss/out` at **step 3340** (the byte-model ratio-10 endpoint),
converted to **BPB** (bits-per-byte).
...
the **ratio-10** (step 3344) checkpoints — matching the BPB table — **except ByteFlow**, whose ratio-10
checkpoint was not saved (only 10032/13376), so it is reported at **ratio-40** (⚠ ~4× more training; its
...
| BPEByte hybrid **leaf · online** | 1.123 | 37.2 | 33.1 | 20.9 | 57.0 | 42.4 | **37.0** |
...
is **offline** (`offlineQ`/`offlineRootQ`) or **online** (`greedyQ`, causal greedy). So *leaf · online* =
leaf-prefill model scored with online question tokenization. `BPEByte root_greedy — online` is the
non-hybrid pure-online model (`c_online`).
...
- **Llama leads by a wide margin** (40.4), carried by ARC-E (42.1) and PIQA (64.3). The best byte-side
  model is **hybrid leaf · online** (37.0).
...
**Caveats.** (1) **ByteFlow is ratio-40, all others ratio-10** — not iso-budget (⚠r40). (2) ARC-C ≈ chance
at this scale. (3) Hybrids scored under their headline Q-mode (leaf·online & online = greedyQ, root·offline
= offlineRootQ); the Q-axis moves the mean ~2 pts. (4) All 0-shot `acc_norm`, `limit=1000`. (5) Pure
