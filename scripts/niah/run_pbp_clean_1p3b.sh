...
OUT="$ROOT/reports/niah/pbp_clean_1p3b"; mkdir -p "$OUT"
...
sniah() { local fam=$1 ck=$2 tag=$3 gpu=$4
  for T in 1 2 3; do
    CUDA_VISIBLE_DEVICES=$gpu $PY scripts/niah/niah_probe.py --family "$fam" --ckpt "$ck" --tag "$tag" \
      --task $T --pbp --pbp_sep ":" --lengths $LENGTHS --depths $DEPTHS --n_per_cell $NPC --batch_size $BATCH \
...
$PY scripts/probes/mk_niah.py --models subword_llama aunet_static byte_greedyroot \
  --n 40 --target_bytes 2048 --needle_counts 1 2 4 8 --pbp --pbp_sep ":" --out "$OUT/mkniah_pbp.jsonl" \
  > "$OUT/mk.log" 2>&1 &
