...
def run():
    ...
    # not a (vacuous) score.
    WINDOW = {"aunet": 8192, "subword": 4096}   # byte: 8192 bytes; Llama: 4096 tokens
    grids = {}
    ...
    fig.text(0.5, -0.02, "n/a = prompt exceeds the model's context window "
             "(byte models: 8192 bytes; Llama: 4096 tokens). At 8k body the byte "
             "models are out-of-window (~8.6 KB > 8192 B); Llama (4096 tok) still "
             "fits and fails on content.",
