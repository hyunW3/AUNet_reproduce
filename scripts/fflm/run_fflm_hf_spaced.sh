#!/usr/bin/env bash
# Official FFLM (synthseq/flipflop) rendered WITH spaces between chars -- isolates
# the dense-vs-spaced format effect. 3 models parallel.
...
PY="$ROOT/lingua/.venv/bin/python"; O="$ROOT/reports/fflm/hf_spaced"; mkdir -p "$O"
...
ff(){ CUDA_VISIBLE_DEVICES=$4 $PY scripts/fflm/fflm_hf.py --family "$1" --ckpt "$ROOT/$2" --tag "$3" \
  --splits val val_dense val_sparse --n_seq 200 --max_reads 8 --batch_size 32 --space --out "$O/hfsp_$3.jsonl" 2>&1 | filt; }
