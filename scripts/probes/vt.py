...
def make_vt(num_hops, num_chains, rng, noise_sents=6, value_digits=7):
    stmts = []
    target_last, target_val = None, None
    for c in range(num_chains):
        V = "".join(rng.choice("0123456789") for _ in range(value_digits))
...
def run_model(tag, n, hop_counts, num_chains, seed, batch, value_digits_list):
    fam, ckpt = ckpt_for(tag)
    gen, tok = build_generator(fam, ckpt, None, 8192)
    rng = random.Random(seed)
    rows = []
    for R in hop_counts:
        for vd in value_digits_list:
            pairs = []
            for _ in range(n):
                prompt, ans = make_vt(R, num_chains, rng, value_digits=vd)
            ...
            rows.append({"tag": tag, "num_hops": R, "num_chains": num_chains,
                         "value_digits": vd, "n": len(res), "exact_match": em})
            print(f"  {tag:16s} hops={R} digits={vd} exact={em:.3f}", flush=True)
    del gen
    return rows
...
def main():
    ...
    ap.add_argument("--value_digits", type=int, nargs="+", default=[7],
                    help="value length(s) in digits to sweep (e.g. 1 2 3 4 5 6)")
    ...
    print(f"VT | hops={args.hop_counts} digits={args.value_digits} "
          f"chains={args.num_chains} n={args.n}")
    for tag in args.models:
        for r in run_model(tag, args.n, args.hop_counts, args.num_chains,
                           args.seed, args.batch_size, args.value_digits):
