...
PY="$ROOT/lingua/.venv/bin/python"; O="$ROOT/reports/statetrack/numericbench"; mkdir -p "$O"
...
one(){ CUDA_VISIBLE_DEVICES=$4 env "$2=$3" $PY scripts/probes/numericbench_eval.py --models "$1" \
  --abilities comparison summary "contextual retrieval" --n_per 150 --out "$O/nb_$1.jsonl" 2>&1 | filt; }
